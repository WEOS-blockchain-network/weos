...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(weos.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(weos.token), contracts::eosio_token_wasm() );
         chain->set_abi(N(weos.token), contracts::eosio_token_abi().data() );
         chain->push_action( N(weos.token), N(create), N(weos.token), mvo()
              ( "issuer", "weos.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(weos.token), N(issue), N(weos.token), mvo()
              ( "to", "weos.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(weos.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, N(weos.token), N(alice)};
   test.init();

   test.transfer( N(weos.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(weos.token), N(bob)};
   test.init();

   test.transfer( N(weos.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(weos.token) );
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(weos.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( actor_blacklist_inline_deferred ) { try {
   ...
   tester1.chain->push_action( N(weos), N(updateauth), N(alice), mvo()
      ( "account", "alice" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   tester1.chain->push_action( N(weos), N(updateauth), N(bob), mvo()
      ( "account", "bob" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   tester1.chain->push_action( N(weos), N(updateauth), N(charlie), mvo()
      ( "account", "charlie" )
      ( "permission", "active" )
      ( "parent", "owner" )
      ( "auth", auth )
   ...
   auto log_trxs = [&](std::tuple<const transaction_trace_ptr&, const signed_transaction&> x) {
      ...
      if( act.account == N(weos) && act.name == N(onblock) ) return;
