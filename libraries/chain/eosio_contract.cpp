 ...
 */
void apply_weos_newaccount(apply_context& context) {
...
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   if( !creator.is_privileged() ) {
      EOS_ASSERT( name_str.find( "weos." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'weos.'" );
   }
...
void apply_weos_setcode(apply_context& context) {
...
void apply_weos_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_weos_updateauth(apply_context& context) {
   ...
   EOS_ASSERT( update.permission.to_string().find( "weos." ) != 0, action_validate_exception,
               "Permission names that start with 'weos.' are reserved" );
...
void apply_weos_deleteauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_weos_linkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_weos_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::eosio_auth_scope );
...
void apply_weos_canceldelay(apply_context& context) {
