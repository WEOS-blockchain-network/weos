...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(weos.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create weos.msig and weos.token
        create_accounts({N(weos.msig), N(weos.token), N(weos.ram), N(weos.ramfee), N(weos.stake), N(weos.vpay), N(weos.bpay), N(weos.saving) });
        // Set code for the following accounts:
        //  - eosio (code: weos.bios) (already set by tester constructor)
        //  - weos.msig (code: weos.msig)
        //  - weos.token (code: weos.token)
        // set_code_abi(N(weos.msig), contracts::eosio_msig_wasm(), contracts::eosio_msig_abi().data());//, &eosio_active_pk);
        // set_code_abi(N(weos.token), contracts::eosio_token_wasm(), contracts::eosio_token_abi().data()); //, &eosio_active_pk);

        set_code_abi(N(weos.msig),
                     contracts::eosio_msig_wasm(),
                     contracts::eosio_msig_abi().data());//, &eosio_active_pk);
        set_code_abi(N(weos.token),
                     contracts::eosio_token_wasm(),
                     contracts::eosio_token_abi().data()); //, &eosio_active_pk);

        // Set privileged for weos.msig and weos.token
        set_privileged(N(weos.msig));
        set_privileged(N(weos.token));

        // Verify weos.msig and weos.token is privileged
        const auto& eosio_msig_acc = get<account_metadata_object, by_name>(N(weos.msig));
        BOOST_TEST(eosio_msig_acc.is_privileged() == true);
        const auto& eosio_token_acc = get<account_metadata_object, by_name>(N(weos.token));
        BOOST_TEST(eosio_token_acc.is_privileged() == true);


        // Create SYS tokens in weos.token, set its manager as eosio
        ...
        create_currency(N(weos.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to weos.system
        issue(N(weos.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(weos.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == name("weos"));
