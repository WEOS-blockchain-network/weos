...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance(N(weos.token), symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action(N(weos.token), N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      {
         create_account( N(weos.token));
         set_code( N(weos.token), contracts::eosio_token_wasm() );

         auto result = push_action(N(weos.token), N(create), mutable_variant_object()
                 ("issuer",       eosio_token)
         ...
         result = push_action(N(weos.token), N(issue), mutable_variant_object()
                 ("to",       eosio_token)
...
const name currency_tester::eosio_token = N(weos.token);
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance(N(weos.token), expected.get_symbol(), N(weos.token));
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action(N(weos.token), N(transfer), mutable_variant_object()
         ("from", eosio_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action(N(weos.token), N(transfer), mutable_variant_object()
                                    ("from", eosio_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
