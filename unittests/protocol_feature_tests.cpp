...
BOOST_AUTO_TEST_CASE( producer_schedule_change_extension_test ) { try {
   ...
   { // ensure producer_schedule_change_extension is rejected
      ...
      bad_block->producer_signature = remote.get_private_key(N(weos), "active").sign(sig_digest);
...
BOOST_AUTO_TEST_CASE( wtmsig_block_signing_inflight_legacy_test ) { try {
   ...
   c.preactivate_protocol_features( {*d} );
   vector<legacy::producer_key> sched = {{N(weos), c.get_public_key(N(weos), "bsk")}};
   ...
   c.block_signing_private_keys.emplace(get_public_key(N(weos), "bsk"), get_private_key(N(weos), "bsk"));
   c.produce_block();
