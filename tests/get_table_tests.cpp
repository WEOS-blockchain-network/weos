...
issue_tokens( TESTER& t, account_name issuer, account_name to, const asset& amount,
              std::string memo = "", account_name token_contract = N(weos.token) )
{
...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   push_action(N(weos.token), N(create), N(weos.token), act );
   ...
   eosio::chain_apis::read_only::get_table_by_scope_params param{N(weos.token), N(accounts), "inita", "", 10};
   ...
   if (result.rows.size() >= 4) {
      BOOST_REQUIRE_EQUAL(name(N(weos.token)), result.rows[0].code);
      BOOST_REQUIRE_EQUAL(name(N(inita)), result.rows[0].scope);
      BOOST_REQUIRE_EQUAL(name(N(accounts)), result.rows[0].table);
      BOOST_REQUIRE_EQUAL(name(N(weos)), result.rows[0].payer);
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("weos", result.rows[0]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("weos", result.rows[1]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("weos", result.rows[2]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("weos", result.rows[3]["payer"].as_string());
   }
   p.show_payer = false;
...
BOOST_FIXTURE_TEST_CASE( get_table_by_seckey_test, TESTER ) try {
   ...
   auto bidname = [this]( const account_name& bidder, const account_name& newname, const asset& bid ) {
      return push_action( N(weos), N(bidname), bidder, fc::mutable_variant_object()
                          ("bidder",  bidder)
                          ("newname", newname)
                          ("bid", bid)
                          );
