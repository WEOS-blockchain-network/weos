...
public:
   ...
   eosio_token_tester() {
      ...
      create_accounts( { N(alice), N(bob), N(carol), N(weos.token) } );
      produce_blocks( 2 );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = eosio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(weos.token), acc, N(accounts), name(symbol_code) );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( N(weos.token), N(create), mvo()
           ( "issuer", issuer)
