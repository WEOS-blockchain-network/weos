...
function usage() {
  ...
  -i DIR      Directory where weos is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the weos data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
...
export WEOS_INSTALL_DIR=${INSTALL_LOCATION:-$WEOS_INSTALL_DIR}

if [[ ! -d "${WEOS_INSTALL_DIR}" ]]; then
   echo "[WEOS installation ${COLOR_YELLOW}NOT${COLOR_NC} found in ${WEOS_INSTALL_DIR}]"
else
   # As of 1.8.0, we're using a versioned directories under home: https://github.com/WEOS/eos/issues/6940
   echo "[WEOS installation found: ${WEOS_INSTALL_DIR}]" && INSTALL_PATHS+=("${WEOS_INSTALL_DIR}") # WEOS_INSTALL_DIR set in .environment
   while true; do
      [[ $NONINTERACTIVE == false ]] && read -p "Do you wish to remove the installation in ${WEOS_INSTALL_DIR}? (y/n) " PROCEED
      case $PROCEED in
         "" ) echo "What would you like to do?";;
         0 | true | [Yy]* )
            ...
            if $FULL; then
               ...
               [[ $ARCH == "Darwin" ]] && INSTALL_PATHS+=("${HOME}/Library/Application\ Support/weos")
               [[ $ARCH != "Darwin" ]] && INSTALL_PATHS+=("${HOME}/.local/share/weos")
            fi
            # Version < 1.8.0; Before we started using ~/weos/1.8.x
            ...
            echo " - WEOS Removal Complete"
            break;;
         1 | false | [Nn]* ) echo " - Cancelled WEOS Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi

echo "[Removing WEOS Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/weos_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
